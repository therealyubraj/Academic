...
let easterEggActivated = false; //boolean for storing whether easte egg is activated
/*
ctx methods:
...
class Snow {
    ...
    resetRandom() {
        ...
        this.size = Math.floor(getRandom(5, 15));
        this.opacity = getRandom(0.3, 1); //the opacity of the particle
        this.yVel = Snow.gravity * this.size; //the yVel is influenced by the size of the snow
        if (easterEggActivated) {
            //randomize color if easter egg is activated
            this.randomizeColor();
        } else {
            this.color = "rgba(255,255,255," + this.opacity + ")"; //color for this snow
        }
    }
    ...
    //randomize the color of this snow for the easterEgg
    randomizeColor() {
        let randR = Math.floor(getRandom(0, 255));
        let randG = Math.floor(getRandom(0, 255));
        let randB = Math.floor(getRandom(0, 255));
        this.color = "rgba(" + randR + "," + randG + "," + randB + "," + 255 + ")";
    }
...
function clearCanvas() {
    ...
    if (easterEggActivated) {
        //clear the canvas with a black background if easter egg is activated
...
/*easter egg code here used for welcome page only*/
let noOfTimeClicked = 0;

function easterEgg() {
    noOfTimeClicked++;
    if (noOfTimeClicked > 12 && !easterEggActivated) {
        document.getElementById("main-container").innerHTML += "WHOA EASTER EGG!!!!!"
        easterEggActivated = true;
        //giv each snow a random color
        snowCollection.forEach((s, i) => {
            snowCollection[i].randomizeColor();
        });

        //change the color for navigation bar buttons to white
        let navButtons = document.getElementsByClassName("nav-bar-button");
        for (let i = 0; i < navButtons.length; i++) {
            navButtons[i].style.color = "#FFFFFF";
        }
    }
}